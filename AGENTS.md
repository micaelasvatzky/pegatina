...
- ✅ **Webhook en panel MP CONFIGURADO (24/09)**: URL `https://pegatina-app.vercel.app/api/mercadopago/webhook` con evento "Order (Mercado Pago)". ⚠️ **La simulación del panel usa `data.id` "123456" (ID falso de la doc de MP) → MP responde 400 `invalid_path_param`, NO 404** — el catch del webhook ahora trata 400 Y 404 como "recurso inválido/inexistente" y responde 200 ok (fix commit 78282f5). La simulación SOLO verifica que la URL responde — nunca marca pedidos reales. La prueba REAL es una compra con cuenta de prueba.
- 🔲 Limpiar pedidos de prueba en Mongo cuando se termine de testear (el del 24/09 `6ab5314d008c88fd543ab12a` ya está marcado **pago.estado "aprobado"** vía backfill verificado contra MP — order ORDTST01M39WH5ZPYR11CEM678RKC3K4, payment PAY01M39WN94GCSM2SX4A2KW9G3Q6).
- ⚠️ **Decisión de Mica (24/09)**: NO consultar el estado del pago al volver del checkout — "mejor saber cuando realmente fue acreditado". La fuente de verdad de acreditación es SOLO el webhook.
- 🔲 Al validar flujo test: activar credenciales de PRODUCCIÓN en Vercel (`MP_ACCESS_TOKEN`, `NEXT_PUBLIC_MP_PUBLIC_KEY`) + `MP_CLIENT_ID`/`MP_CLIENT_SECRET` producción por el OAuth de artistas (Mica los pega directo en Vercel, nunca en el chat). Mica ya pegó `MP_NOTIFICATION_SECRET` en Vercel (el webhook dejó de dar 401 de firma).
