...
### Reescritura fiel a Stitch (PENDIENTE — segunda pasada)
- 🔲 **`StickerCard.tsx`**: estilo Stitch — category chip de color por categoría, autor, material, precio + "ARS", stepper + Agregar, modo `readOnly` (página artista).
- 🔲 **`(store)/page.tsx`** (landing): hero con badge + headline palabra destacada + pills + 3-panel showcase con datos reales + search strip; destacados con cards Stitch; categorías tiles; notification tape.
- 🔲 **`(store)/catalogo/page.tsx`**: breadcrumbs, title bar, sidebar filters, grid StickerCards, pagination real.
- 🔲 **`(store)/producto/[id]/page.tsx`**: 2-col, galería real, badges, info card, ficha técnica real, artist spotlight real, related reales.
- 🔲 **`(store)/artista/[usuario]/page.tsx`**: hero banner + card flotante, métricas reales, grid readOnly.
- 🔲 **Footer multi-column** en `(store)/layout.tsx` con links reales y copy sin "microediciones".
- 🔲 `npx tsc --noEmit` al terminar.
